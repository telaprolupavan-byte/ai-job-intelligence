...
| AJI-029 | Zero-Cost Job Provider Evaluation & Approval: research only. **No production provider was approved, none was integrated, and no API credentials were added.** Himalayas remains **unverified: do not integrate**. USAJOBS and the other candidates remain subject to the findings in the AJI-029 research report, which is not committed to this repository (the earlier AJI-028 findings are in `services/job_discovery/README.md`, "Source research status (not approved)"). AJI-030 deliberately proceeded on the controlled development dataset instead of waiting for a provider. | Research complete / Provider approval pending | — |
...
| AJI-031 | AI Evaluation & Quality Framework: a small, synthetic, hand-labelled evaluation dataset (`services/ai_evaluation/dataset/v1`), capability-specific metrics, evidence-grounding and prompt-injection checks that reuse the production mechanisms, a repeatable command (`python -m services.ai_evaluation.run_eval`, `--live` for the LLM stages), and a committed deterministic baseline. Evaluation only: no production prompt, model, extraction, matching or scoring change. Branch `pavant/focused-bohr-vzzgzy`. | Claimed | — |
| AJI-032B | Codebase organization, documentation and test support only: `docs/CODEBASE_MAP.md`, shared backend test helpers in `tests/support/`. No production, API, schema, AI or frontend change. Branch `pavant/brave-fermi-d0jwjk`. | Claimed | — |
