...
## Job Search / Discovery foundation (AJI-023, Job Search)

**Ticket-number note:** this work was requested as "AJI-023 — Job Search /
Discovery". The same number was already used by "Job Intelligence →
Application Decision Workflow (AJI-023)" above. Code comments say
"AJI-023 (Job Search)" to keep the two apart. The Product Owner should
confirm the number.

### What already existed (reused, unchanged)

Most of the requested foundation predates this ticket and was reused as-is:
the provider adapter boundary, `DiscoveredJob` normalization, validation,
identity-based deduplication, the synthetic test provider (all "Job
Discovery Product Pipeline (AJI-024)"); the `GET /jobs` listing with
keyword/employment/work-arrangement/location filters; `GET /jobs/{job_id}`;
and the `/jobs` search form, loading/empty/error states, cards, and
"Open job". Search stays **ingest-then-search-locally**. Providers are read
by scheduled discovery runs, and a user's search queries NERO's own
normalized `jobs` table. No search-time provider call was added: that would
be a second discovery path and a real architecture change. Approving a
provider still means one adapter plus one branch in
`build_configured_source()`, and nothing in search changes.

**Real vs. stubbed today:** the Greenhouse adapter is real code but is
**not configured** (no board is approved, see "Why unconfigured by
default"). The only runnable provider is the synthetic `test_fixture`,
double-gated and labelled "Test data". No provider credentials exist in
the repository and none were invented.

### What AJI-023 (Job Search) added

- **Validated search criteria.** `apps/api/services/job_listing.py` owns
  the contract that `GET /jobs` and `GET /jobs/priority` both declare,
  because they must keep identical filter semantics:
  - `employment_type` / `remote_type` are `Literal`s of exactly the values
    discovery normalization can store. A test pins them to
    `services/job_discovery/normalizer.py`.
  - `search` / `location` are capped at `SEARCH_TEXT_MAX_LENGTH` (200).
  - Anything else is a FastAPI 422. Before this ticket, a typo'd filter
    silently returned zero jobs.
  - Free text is trimmed and whitespace-collapsed; blank means no criterion.
  - `%`, `_` and `\` are escaped, so they are searched literally instead
    of acting as `LIKE` wildcards.
- **`source_job_id`** is an additive field on every job response. It is
  the provider's own posting id (`Job.external_job_id`), null when absent,
  and never generated by NERO.
- **Search UI hardening (`/jobs`).**
  - The pure criteria helpers live in `lib/job-search.ts`: the vocabulary
    mirror, `cleanCriterion`, and `toSearchError`.
  - Inputs carry `maxLength`. A URL filter value the API would reject is
    dropped instead of sent.
  - `getJobs` now uses `apiRequest` with a 20s timeout, so a hung request
    becomes a retryable error instead of an endless skeleton, and the
    error carries its HTTP status.
  - On failure the previous results are cleared, so they never read as
    results for the new criteria. A 422 says the criteria weren't
    accepted, with no pointless retry. Any other failure offers "Try Again".
- **Job details.** An opened job's card renders
  `components/app/job-details.tsx` in place of the listing's 3-line
  description:
  - company, location, work arrangement, employment type, compensation,
    posted date, contract length/worker type (contract roles only), and
    source job id;
  - the full description, requirements and responsibilities.
  - A key fact the source didn't state reads "Not stated", and sections it
    didn't provide are omitted. Cards also show the posted date when one
    exists.
- **No fabrication fix.** `formatSalary` (moved to `lib/job-format.ts`)
  used to show "USD" when the source gave no currency. It now shows the
  amount alone.

### Duplicates

Identity and dedup remain AJI-006/AJI-024's (`source` + provider id, else
the fallback fingerprint): within-batch repeats are counted and skipped,
and repeat runs update in place. `tests/test_job_search_api.py` checks this
end to end: after two fixture runs (whose batch repeats a record), search
shows that posting once. **Not handled:** the same real-world posting
arriving from two *different* providers. Only one provider can be active
today, and choosing which source wins is a product decision.

### Authentication

Unchanged by design:
- `GET /jobs` stays public (the AJI-011 rule; the landing page uses it)
  and never includes private submissions for anonymous callers or other
  users.
- `GET /jobs/{job_id}` and `GET /jobs/priority` require a bearer token.
- `/jobs` sits behind `AuthGate`.

### Not included (intentionally)

Full-Time vs Contract classification, resume/job matching changes, AI job
understanding, auto-apply, application-tracking changes, cross-provider
dedup, search over description text, and any real provider selection.

### Testing

- `tests/test_job_search_api.py` (real DB, over HTTP): valid and combined
  criteria, company search, blank/whitespace/wildcard handling, every
  invalid-criteria 422 (including on the priority view), the
  vocabulary↔normalizer pin, empty results, missing optional fields staying
  null, duplicates through the real pipeline, detail retrieval and 404s,
  inactive jobs, auth on detail/priority, private-job isolation, and a
  database failure returning a plain 500 without internals.
