...
class ResumeVersion(Base):
    ...
    # "upload" (the user uploaded a document), "improvement" (generated
    # by AJI-021 from user-approved, user-authored improvement content
    # for one job), or "general_improvement" (generated by AJI-027 General
    # Resume Intelligence from user-approved, user-authored content, with
    # no job involved). Never inferred from `parent_version_id` being
    # set, so provenance stays legible even if a parent row is later
    # deleted.
...
class GeneralResumeAssessment(Base):
    """One General Resume Score assessment of one exact `ResumeVersion`
    (AJI-027, General Resume Intelligence).

    Job-independent by construction: this table has no `job_id`, no
    requirement snapshot, and no ATS reference. The score is a pure,
    deterministic function of the version's own text (see
    apps/api/services/general_resume/scoring.py), so it is never an ATS
    score and never an estimate of fit for any posting. It is kept
    entirely separate from `AtsAlignmentResult`, `JobMatchResult`,
    `RequirementIntelligence` and `GapAnalysis`.

    AI usage is optional enrichment of the explanation/guidance text of
    already-detected improvements only; the score, the improvement list,
    and every improvement's type are decided before any AI call and are
    identical whether the AI succeeds or fails. It therefore follows the
    AI-derived versioning convention (analysis/analyzer/prompt +
    model_provider/model_name), plus `scoring_version` for the formula.

    Insert-only, like every other analysis artifact: a re-assessment is
    a new row, never an update. The unique constraint makes the POST
    idempotent per (user, version, pipeline versions), and resolves a
    concurrent double-submit to one row.
    """

    __tablename__ = "general_resume_assessments"
    ...
    # Copied from the assessed version so a stored assessment records
    # exactly which text it scored.
    content_fingerprint: Mapped[str] = mapped_column(
        String(64),
        nullable=False,
    )
    ...
    # The General Resume Score (0-100). No band, label, or threshold is
    # stored or derived anywhere.
    overall_score: Mapped[float] = mapped_column(Float, nullable=False)

    # "complete" or "partial" (the optional AI enrichment failed and every
    # explanation/guidance field fell back to its deterministic
    # template). Never affects the score or readiness.
    generation_status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
    )

    # See apps/api/services/general_resume/contracts.py::
    # GeneralResumeAssessmentResult for the exact shape.
...
class GeneralResumeReview(Base):
    """One submitted approve/reject decision set over one
    `GeneralResumeAssessment` (AJI-027).

    - Every approved decision carries the user's own text; nothing here
      or anywhere in General Resume Intelligence writes resume content
      on the user's behalf.
    - Rejections are recorded here and carry forward to descendant
      versions while the same unchanged issue remains (same improvement
      id - see general_resume/improvements.py).
    - `child_resume_version_id` is NULL when nothing was approved: a
      review that rejects everything is valid and creates no version.
    - The parent version is never modified; a child is always
      `source = "general_improvement"`, `is_master = False`.

    Mutability matches AJI-021's documented deviation: only the recheck
    fields change (a failed recheck can be retried), and once
    `recheck_assessment_id` is set it is never re-pointed. The decisions,
    fingerprint and child version never change.
    """

    __tablename__ = "general_resume_reviews"
    ...
    # The baseline assessment the user reviewed.
    assessment_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("general_resume_assessments.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    child_resume_version_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("resume_versions.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    ...
    # Applying decisions is deterministic text assembly (no AI call), so
    # the review follows the single `engine_version` convention.
    ...
    # The persisted decision records - see contracts.py::
    # ReviewDecisionRecord.
    decisions: Mapped[list] = mapped_column(JSONB, nullable=False)

    # "not_required" (nothing approved, no version), "pending",
    # "complete", or "failed". A failure never removes the child version.
    recheck_status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
    )

    recheck_error: Mapped[str | None] = mapped_column(Text, nullable=True)

    recheck_assessment_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("general_resume_assessments.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
